...
NSString const *CWBlurBtnKey = @"CWFadeBtnKey";
...
//
//}
//-(void)setBlurBtn:(UIButton *)blurBtn
//{
//    self.blurBtn = blurBtn;
//}
- (void)addBlurBgBtn {
    UIImageView *blurView = [UIImageView new];
  UIButton  *blurBtn = [UIButton new];
    if (UIDeviceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation)) {
        blurBtn.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
    } else {
        blurBtn.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.height, [UIScreen mainScreen].bounds.size.width);
    }
//    [self addBlurView];
//    [self addBlurView];
//    [self addBlurView];
...
//    [self addBlurView];
    //blurView.alpha = 0.0f;
    [blurBtn setBackgroundImage:[self getBlurredImage:[self getScreenImage]] forState:UIControlStateNormal];
    [blurBtn setBackgroundImage:[self getBlurredImage:[self getScreenImage]] forState:UIControlStateHighlighted];
    
    [blurBtn addTarget:self action:@selector(removeBlurBtn) forControlEvents:UIControlEventTouchUpInside];
    
    blurView.image = [self getBlurredImage:[self getScreenImage]];
    [self.view addSubview:blurBtn];
    //[self.view bringSubviewToFront:self.popupViewController.view];
        objc_setAssociatedObject(self, &CWBlurBtnKey, blurBtn, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
-(void)removeBlurBtn
{
...
-(UIButton *)blurBtn
{
     return objc_getAssociatedObject(self, &CWBlurBtnKey);
}
...
-(UIButton *)blurView
{
    return objc_getAssociatedObject(self, &CWBlurViewKey);
}

-(void)setBlurView:(UIImageView *)blurView
{
    objc_setAssociatedObject(self, &CWBlurViewKey, blurView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
@end
