...
@interface FindPwdViewController ()<MZTimerLabelDelegate,UITextFieldDelegate,UIAlertViewDelegate>
{
    CGFloat frameHeight;
    LPPopup *popup;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        alterLabel = [[UILabel alloc]initWithFrame:CGRectMake(20, 0, [UIScreen mainScreen].applicationFrame.size.width, kLabelHeight*kAutoSizeScaleY)];
        ...
        __weak FindPwdViewController *weakSelf = self;
...
-(void)rePassCode
{
    ...
    if (selfPhone != nil) {
        request = [NSURLRequest requestWithPath:@"userRegister" params:@{@"phone":selfPhone,@"type":@"verification"}];
...
-(void)sendRegister
{
    ...
    if (![pwdCell.textField.text isEqualToString:ensurePwdCell.textField.text]) {
        ...
        NSURLRequest *request = [NSURLRequest requestWithPath:@"userRegister" params:@{@"phone":[[NSUserDefaults standardUserDefaults] objectForKey:@"findPwdPhone"],@"verification": codeCell.textField.text,@"type":@"getverification",@"type1":@"find_password"  }];
        ...
        [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue currentQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
            if (connectionError==nil) {
                ...
                if ([str isEqualToString:@"code success"]) {
                    ...
                    NSURLRequest *request = [NSURLRequest requestWithPath:@"userRegister" params:@{@"uAccount":[[NSUserDefaults standardUserDefaults] objectForKey:kPhone],@"upassword": pwdCell.textField.text,@"type":@"find_password"                              }];
                    ...
                    [SVProgressHUD dismiss];
                    [_indictor stopAnimating];
                    popup = [LPPopup popupWithText:@"验证错误!"];
                    popup.popupColor = [UIColor blackColor];
                    popup.alpha = 0.8;
                    popup.textColor = [UIColor whiteColor];
                    popup.font = kDetailContentFont;
...
-(void)finalRegister
{
    ...
    if([codeCell.textField.text isEqualToString:@""]){
