...
@implementation FPwdViewController
...
- (void)addSubViews
{
    ...
    self.loginBtn = [[UIButton alloc]initWithFrame:CGRectMake(([UIScreen mainScreen].applicationFrame.size.width - kBtnWidth)/2, kHeightMargin +_phoneText.frame.origin.y+_phoneText.frame.size.height, kBtnWidth, kBtnHeight)];
...
//    FindPwdViewController *findNext = [[FindPwdViewController alloc]init];
//    if ([self.phoneText.placeholder isEqualToString:@"请输入手机号"]&&[self.phoneText.text isEqualToString:@""]) {
//        popup = [LPPopup popupWithText:@"请输入手机号!"];
...
//               completion:nil];
//    }else{
//    if (findNext.passPhone) {
//        findNext.passPhone(self.phoneText.text);
//    }
//    [self.navigationController pushViewController:findNext animated:YES];
//    }
     ...
     [SVProgressHUD showWithStatus:@"加载中" maskType:SVProgressHUDMaskTypeNone];
        ...
        if (![self checkTel:self.phoneText.text]) {
            ...
            NSURLRequest *request = [NSURLRequest requestWithPath:@"userRegister" params:@{@"phone":self.phoneText.text,@"type":@"verification",@"type1":@"find_password"                                                  }];
           [[NSUserDefaults standardUserDefaults]setObject:self.phoneText.text forKey:@"findPwdPhone"];
            NSLog(@"%@",request);
...
- (void)connection:(NSURLConnection *)aConn didReceiveData:(NSData *)data
{
    ...
    if ([responesStr isEqualToString:@"verification success"]) {
        ...
        [self.navigationController pushViewController:findNext animated:YES];
    }else if([responesStr isEqualToString:@"user unreg"]){
        [SVProgressHUD dismiss];
        popup = [LPPopup popupWithText:@"用户还不存在,请先注册!"];
        popup.popupColor = [UIColor blackColor];
        popup.alpha = 0.8;
        popup.textColor = [UIColor whiteColor];
        popup.font = kDetailContentFont;
        ...
        [SVProgressHUD dismiss];
        popup = [LPPopup popupWithText:@"找回失败，请稍后再试！"];
        popup.popupColor = [UIColor blackColor];
        popup.alpha = 0.8;
        popup.textColor = [UIColor whiteColor];
        popup.font = kDetailContentFont;
